import { LeilaoResponse, LeilaoResumo, LeilaoRelatorioResumo } from "@/types/leilao";
...
export async function buscarLeiloesAbertos(): Promise<LeilaoResponse> {
    ...
    const fs = require('fs');
    try { fs.appendFileSync('debug.log', `[${new Date().toISOString()}] [API] Fetching: ${fullUrl}\n`); } catch (e) { }
    ...
    if (!res.ok) {
        ...
        try {
            fs.appendFileSync('debug.log', `[${new Date().toISOString()}] [API] Error Status: ${res.status}\n`);
            fs.appendFileSync('debug.log', `[${new Date().toISOString()}] [API] Error Body: ${text}\n`);
        } catch (e) { }
        throw new Error(`Erro ao buscar leilões: ${res.status} - ${text.substring(0, 100)}`);
    }
...
export async function buscarRelatorioResumo(id: number): Promise<LeilaoRelatorioResumo> {
    const token = await obterToken();
